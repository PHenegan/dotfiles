...
node_id=$($HOME/.config/niri/scripts/get_node_id.sh $1)
...
if [ "$muted" ]; then
  pw-play $HOME/.config/niri/resources/discord-mute.mp3 --volume 0.4
else
  pw-play $HOME/.config/niri/resources/discord-unmute.mp3 --volume 0.4
fi
